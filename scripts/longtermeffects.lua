            ...
            for _, nodeEffect in pairs(DB.getChildren(nodeCT, 'effects')) do
                ...
                if nActive ~= 0 then
                    ...
                    elseif bHasDuration then
                        DB.setValue(nodeEffect, 'duration', 'number',
                                    nDuration - nRounds);
                    end
                end
            end
...
local function filterTable(tTable, filterFunction)
    ...
    for key, value in pairs(tTable) do
        if filterFunction(value) then table.insert(tFiltered, key, value) end
    end
...
local function actorRequiresSlowMode(rActor, arrSEffects)
    ...
    if StringManager.contains(arrSEffects, 'FHEAL') then
        if sActorHealth ~= ActorHealthManager.STATUS_HEALTHY and sActorHealth ~=
            ActorHealthManager.STATUS_DEAD then return true; end
    end
    return false;
end
...
local function getIsStableAndEffectsToCheck(nodeCT)
    ...
    for _, nEffect in pairs(DB.getChildren(nodeCT, 'effects')) do
        ...
        if string.lower(sEffect) == 'stable' then
            ...
            local splitSimulatedEffectComps =
                filterTable(splitEffectComps, effectTypeShouldBeChecked);
...
local function nextRound_new(nRounds, bTimeChanged)
    ...
    if nodeActive then
        ...
        msg.text = '[' .. Interface.getString('combat_tag_round') .. ' ' ..
                       nCurrent .. ']';
...
function onInit()
    ...
    if sRuleset == '3.5E' or sRuleset == 'PFRPG' or sRuleset == 'PFRPG2' or
        sRuleset == '5E' then
    ...
    OptionsManager.registerOption2('TIMEROUNDS', false, 'option_header_game',
                                   'opt_lab_time_rounds', 'option_entry_cycler',
                                   {
